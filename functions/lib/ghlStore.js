...
export function installDocIdForAgency(companyId, clientIdFingerprint) {
    if (companyId && String(companyId).trim())
        return `agency_${companyId}`;
    return `agency_byClient_${clientIdFingerprint ?? "unknown"}`;
}
export function installDocIdForLocation(locationId) {
    return `location_${locationId}`;
}
export async function upsertInstallTokens(docId, payload) {
...
export async function readInstallByAgencyId(companyId) {
...
export async function updateTokens(docId, tokens) {
